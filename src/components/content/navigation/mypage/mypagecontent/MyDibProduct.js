...
export class MyDibProduct extends React.Component{
    ...
    componentDidMount=()=>{
        ...
        axios({
            method: 'get',
            url: process.env.API_URL + '/api/auth/likeProduct' + query,
    ...
    deleteChecked = async() => {
        ...
        axios({
            method: 'get',
            url: `${process.env.API_URL}/api/auth/likeProduct?isCosmetic=${this.state.mainCategory}&page=${this.state.currentPage}`,
            headers: TokenUtils.getTokenRequestHeader(token)
        })
        .then((res) => {
            this.setState({
